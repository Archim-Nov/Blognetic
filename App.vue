...
<template>
  <div
    class="container mx-auto px-6 py-8 max-w-[1200px] wobbly-box wobbly-box-container my-8 min-h-[90vh]"
  >
    <BlogHeader
    ...
    <BlogHero
      :currentLocation="
        LOCATIONS[uiStore.lang][gameStore.gameState.locationIndex]
      "
      ...
      :isResting="gameStore.isResting"
      @continue="gameStore.continueJourney"
    />
    ...
    <template v-else>
      <div
        class="flex items-center justify-center gap-4 my-12 text-[var(--text-title)] font-title text-2xl"
      >
        <div
          class="h-1 flex-1 bg-[var(--border-color)] rounded-full animate-[squiggly-anim_0.3s_linear_infinite] max-w-[100px]"
        ></div>
    ...
    <footer
      class="wobbly-box bg-card-context p-6 mt-16 rounded-[var(--radius-md)] text-center font-title text-[var(--text-main)]"
    >
